...
class BannermakerInstallerScript
{
	function postflight( $type, $parent )
	{
        if(!JPluginHelper::getPlugin('content', 'bannermaker') && JFile::exists(JPATH_SITE . DIRECTORY_SEPARATOR . 'plugins' . DIRECTORY_SEPARATOR . 'content' . DIRECTORY_SEPARATOR . 'bannermaker' . DIRECTORY_SEPARATOR . 'bannermaker.php'))
		{
			$db = JFactory::getDBO();
			$query = "UPDATE #__extensions SET enabled='1' WHERE element='bannermaker'";
			$db->setQuery($query);
			$db->query();
		}
